    ...
    <main>
        <div class="container" style="min-height: 80vh;">
            <c:choose>
                        ...
                        <table class="w-100">
                            ...
                            <tbody>
                                <c:forEach var="post" items="${adminPosts}">
                                    <tr>
                                        <td>${post.id}</td>
                                        <td>${post.cantidadVisitas}</td>
                                        <td>${post.fechaCreacion}</td>
                                        <td>${post.sello}</td>
                                        <td>${post.usuario.id}</td>
                                        <td>
                                            <form:form action="modificarPost" method="get">
                                                <input type="hidden" name="idPost" value="${post.id}">
                                                <button class="btn btn-primary">Modificar</button>
                                            </form:form>
                                        </td>
                                        <td>
                                            <form:form action="eliminarPost" method="post">
                                                <input type="hidden" name="idPost" value="${post.id}">
                ...
                <c:when test="${adminPostsNull == true}">
                    <div class="row">
                        <div class="col-12 d-flex flex-row bg-white m-4 rounded-3 border border-2 border-black">
                            <h2 class="m-auto p-3">Aún no hay datos en la tabla Posts</h2>
                        </div>
                    </div>
                </c:when>
            </c:choose>
