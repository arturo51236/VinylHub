...
<body>
    ...
    <main>
        <div class="container" style="min-height: 80vh;">
            <c:choose>
                <c:when test="${adminDiscos != null}">
                    <div class="d-flex bg-white text-center rounded-3 border border-2 border-black m-3 p-3">
                        <table class="w-100">
                            <thead class="p-3">
                            <tr>
                                <th class="py-2">ID</th>
                            ...
                            <tbody>
                            <c:forEach var="disco" items="${adminDiscos}">
                                <tr>
                                    <td class="px-3">${disco.id}</td>
                                    <td class="px-3">${disco.artista}</td>
                                    <td class="px-3">${disco.nombre}</td>
                                    <td class="px-3">${disco.anio}</td>
                                    <td class="px-3">${disco.foto}</td>
                                    <td class="px-3">${disco.pais}</td>
                                    <td class="px-3">${disco.notas}</td>
                                    <td class="px-3">${disco.formato.id}</td>
                                    <td class="px-3">${disco.genero.id}</td>
                                    <td class="px-3">${disco.post.id}</td>
                                    <td class="px-3">
                                        <form:form action="modificarDisco" method="get">
                                            <input type="hidden" name="idDisco" value="${disco.id}">
                ...
                <c:when test="${adminDiscosNull == true}">
                    <div class="row">
                        <div class="col-12 d-flex flex-row bg-white m-4 rounded-3 border border-2 border-black">
                            <h2 class="m-auto p-3">Aún no hay datos en la tabla Discos</h2>
                        </div>
                    </div>
                </c:when>
            </c:choose>
