    ...
    <main>
        <div class="container" style="min-height: 80vh;">
            <c:choose>
                <c:when test="${adminGeneros != null}">
                    <div class="d-flex bg-white text-center rounded-3 border border-2 border-black m-3 p-3">
                        <table class="w-100">
                            ...
                            <tbody>
                            <c:forEach var="genero" items="${adminGeneros}">
                                <tr>
                                    <td>${genero.id}</td>
                                    <td>${genero.nombre}</td>
                                    <td>
                                        <form:form action="modificarGenero" method="get">
                                            <input type="hidden" name="idGenero" value="${genero.id}">
                                            <button class="btn btn-primary">Modificar</button>
                                        </form:form>
                                    </td>
                                    <td>
                                        <form:form action="eliminarGenero" method="post">
                                            <input type="hidden" name="idGenero" value="${genero.id}">
                ...
                <c:when test="${adminGenerosNull == true}">
                    <div class="row">
                        <div class="col-12 d-flex flex-row bg-white m-4 rounded-3 border border-2 border-black">
                            <h2 class="m-auto p-3">Aún no hay datos en la tabla Géneros</h2>
                        </div>
                    </div>
                </c:when>
            </c:choose>
            <div class="row m-3">
                <div class="col-8 d-flex flex-row m-auto">
                    <form:form action="insertarGenero" method="get" class="w-100">
                        <button type="submit" class="btn btn-success p-3 rounded-4 w-100">Añadir género</button>
                    </form:form>
                </div>
            </div>
        </div>
