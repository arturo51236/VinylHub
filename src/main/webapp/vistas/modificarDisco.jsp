...
<body>
    ...
    <main>
        <div class="container" style="min-height: 80vh">
            <div class="row">
                <div class="col-8 d-flex flex-row mt-2 m-auto">
                    <%--@elvariable id="discoM" type="es.arturonb.modelos.Disco"--%>
                    <form:form method="post" action="modificarDisco" class="w-100 text-center" modelAttribute="discoM">
                        <div class="row">
                            <div class="col-2 d-flex flex-column">
                                <h4 class="m-auto">Foto: <span style="color: red">*</span></h4>
                                <div class="mt-2">
                                    <img src="${pageContext.request.contextPath}/static/images-discos/${disco.foto}" class="w-100 h-100" id="aniadirFoto">
                                    <input type="file" style="display: none;" id="inputFileImage">
                                    <form:input value="${disco.foto}" type="text" path="foto" class="d-none"></form:input>
                                </div>
                            </div>
                            <div class="col-10 mt-4">
                                ...
                                <form:input type="text" class="form-control" id="inputArtista" required="" value="${disco.artista}" path="artista"></form:input>
                                ...
                                <form:input type="text" class="form-control" id="inputNombre" required="" value="${disco.nombre}" path="nombre"></form:input>
                                <label for="inputAnio" class="col-form-label m-auto text-center">Año de lanzamiento: <span style="color: red">*</span></label><br>
                                <form:input type="number" class="form-control" id="inputAnio" min="1948" max="2024" required="" value="${disco.anio}" path="anio"></form:input>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-12">
                                <label for="inputPais" class="col-form-label m-auto text-center mt-2">País de lanzamiento: <span style="color: red">*</span></label><br>
                                <form:input type="text" class="form-control" id="inputPais" required="" value="${disco.pais}" path="pais"></form:input>
                                <label for="inputGenero" class="col-form-label m-auto text-center">Género: <span style="color: red">*</span></label><br>
                                <form:select id="inputGenero" class="form-select" path="genero.id">
                                    <c:forEach var="gener" items="${generos}">
                                        <option value="${gener.id}" ${disco.genero.nombre eq gener.nombre ? 'selected' : ''}>${gener.nombre}</option>
                                    </c:forEach>
                                </form:select>
                                <label for="inputFormato" class="col-form-label m-auto text-center">Formato: <span style="color: red">*</span></label><br>
                                <form:select id="inputFormato" class="form-select" path="formato.id">
                                    <c:forEach var="forma" items="${formatos}">
                                        <option value="${forma.id}" ${disco.formato.nombre eq forma.nombre ? 'selected' : ''}>${forma.nombre}</option>
                                    </c:forEach>
                                </form:select>
                                <label for="inputNotas" class="col-form-label m-auto text-center">Notas:</label><br>
                                <form:input type="text" class="form-control mb-4" id="inputNotas" value="${disco.notas}" path="notas"></form:input>
                                <form:input type="number" path="id" value="${disco.id}" class="d-none"></form:input>
                                <form:input type="number" path="post.id" value="${disco.post.id}" class="d-none"></form:input>
                                <form:button class="btn btn-success p-3 rounded-4 w-100" id="btnCrearDisco">Modificar</form:button>
                            </div>
                        </div>
                    </form:form>
                </div>
