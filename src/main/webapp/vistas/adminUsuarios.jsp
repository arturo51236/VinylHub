...
<body>
    ...
    <main>
        <div class="container" style="min-height: 80vh;">
            <c:choose>
                <c:when test="${adminUsuarios != null}">
                    <div class="d-flex bg-white text-center rounded-3 border border-2 border-black m-3 p-3">
                        <table class="w-100">
                            ...
                            <tbody>
                            <c:forEach var="usuario" items="${adminUsuarios}">
                                <tr>
                                    <td>${usuario.id}</td>
                                    ...
                                    <td>${usuario.rol}</td>
                                    <td>${usuario.wantlist.id}</td>
                                    <td>
                                        <form:form action="modificarUsuario" method="get">
                                            <input type="hidden" name="idUsuario" value="${usuario.id}">
                                            <button class="btn btn-primary">Modificar</button>
                                        </form:form>
                                    </td>
                                    <td>
                                        <form:form action="eliminarUsuario" method="post">
                                            <input type="hidden" name="idUsuario" value="${usuario.id}">
                ...
                <c:when test="${adminUsuariosNull == true}">
                    <div class="row">
                        <div class="col-12 d-flex flex-row bg-white m-4 rounded-3 border border-2 border-black">
                            <h2 class="m-auto p-3">Aún no hay datos en la tabla Usuarios</h2>
                        </div>
                    </div>
                </c:when>
            </c:choose>
