...
<head>
    ...
    <title>VinylHub - Mi wantlist</title>
...
<body>
    ...
    <main>
        <div class="container" style="min-height: 80vh;">
            <c:choose>
                <c:when test="${miWantlist != null}">
                    <c:forEach var="post" items="${miWantlist}">
                        <div class="row m-3">
                            <div class="col-12 d-flex flex-row bg-white justify-content-between align-items-center p-2 rounded-3 border border-2 border-black">
                                <img src="${pageContext.request.contextPath}/static/images-discos/${post.disco.foto}" class="card-img-top rounded-2" id="img-disco">
                                <a href="../verPost/${post.id}" style="font-size: 23px; text-decoration: none">${post.disco.artista} - ${post.disco.nombre}</a>
                                <form:form action="../eliminarDeWantlist" method="post">
                ...
                <c:when test="${wantlistNull == true}">
                    <div class="row">
                        <div class="col-12 d-flex flex-row bg-white m-4 rounded-3 border border-2 border-black">
                            <h2 class="m-auto p-3">Aún no dispones de discos en tu wantlist</h2>
                        </div>
                    </div>
                </c:when>
            </c:choose>
